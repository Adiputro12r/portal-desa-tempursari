...
/**
 * BeritaClient — Client Component
 *
 * Menerima semua artikel dari Server Component parent sebagai prop `articles`.
 * Handle filter kategori + search query secara client-side (instant, tanpa fetch ulang).
 */
...
export default function BeritaClient({ articles }) {
  ...
  const filtered = articles.filter((b) => {
    ...
    const matchSearch =
      ...
      (b.konten?.replace(/<[^>]*>/g, "").toLowerCase().includes(q));
    const matchCat =
