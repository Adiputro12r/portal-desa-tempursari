...
import { ShoppingBag, Compass, MapPin, MessageSquare, ExternalLink } from "lucide-react";
...
/**
 * UmkmWisataClient — komponen interaktif untuk halaman UMKM & Wisata.
 * Menerima `initialUmkm` dan `initialWisata` dari Server Component parent.
 * Hanya bertugas mengelola state tab dan render UI.
 */
export default function UmkmWisataClient({ initialUmkm = [], initialWisata = [] }) {
  const [activeTab, setActiveTab] = useState("semua");

  // Simpan ke memoryCache supaya prefetcher tidak perlu fetch ulang
  if (initialUmkm.length > 0 && !memoryCache[CACHE_KEY_UMKM]) {
    memoryCache[CACHE_KEY_UMKM] = initialUmkm;
  }
  if (initialWisata.length > 0 && !memoryCache[CACHE_KEY_WISATA]) {
    memoryCache[CACHE_KEY_WISATA] = initialWisata;
  }

  const handleWaContact = (umkm) => {
    const cleanPhone = (umkm.kontak || "").replace("+", "");
    window.open(
      `https://wa.me/${cleanPhone}?text=Halo%20${umkm.pemilik},%20saya%20tertarik%20dengan%20produk%20${umkm.nama_usaha}%20Anda.`,
      "_blank"
    );
  };
